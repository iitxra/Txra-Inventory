...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			TriggerClientEvent("Txra-inventory:client:closeinv", id)
...
RegisterNetEvent('Txra-inventory:server:SaveStashItems', function(stashId, items)
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info) then
				...
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('Txra-inventory:client:giveAnim', src)
				TriggerClientEvent('Txra-inventory:client:giveAnim', target)
			else
...
QBCore.Functions.CreateCallback('Txra-inventory:server:GetStashItems', function(source, cb, stashId)
                ...
                for slot, item in pairs(Player.PlayerData.items) do
                    if item and item.name then
                        if not itemCount[item.name] then
                            itemCount[item.name] = {slot = slot, count = 0}
                        end
                        itemCount[item.name].count = itemCount[item.name].count + 1
                        
                        if itemCount[item.name].count > 1 then
                            table.insert(itemsToRemove, slot)
                        end
                    end
                end
                
                for _, slot in pairs(itemsToRemove) do
                    Player.PlayerData.items[slot] = nil
                end
